...
class ZbrojniNalog extends Model
{
    ...
    public static function glavaSloga($id){
        $results = DB::select("select zn.VrstaNalogaId, zn.id, par.Naziv, zr.IBAN, nal.DatumIzvrsenja, v.Alfa ,zr.IBAN,sum(nal.Iznos) suma,count(*) brojNaloga
    ...
    public static function stavke($vrstaNaloga, $znid, $iban, $datum){
        ...
        $results = DB::select("SELECT n.ZbrojniNalogId, n.Opis, n.Iznos, n.DatumIzvrsenja, plzr.IBAN ibanPlatitelja , mo.Vrijednost modelOdobrenja, n.BrojOdobrenja, przr.IBAN ibanPrimatelja, p.Naziv nazivPrimatelja,  mz.Vrijednost modelZaduzenja, n.BrojZaduzenja 
                  FROM Nalozi n 
                  join Valute v
                    on v.id = n.ValuteId
